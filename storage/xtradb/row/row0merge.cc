	...
	uint			buf_size)
{
	...
	int rc = encryption_scheme_encrypt(input_buf,
					   buf_size,
					   crypted_buf, &dstlen,
					   crypt_data,
					   crypt_data->key_version_alter,
					   space, offset, 0);

	if (!((rc == MY_AES_OK) && (dstlen == buf_size))) {
		ib_logf(IB_LOG_LEVEL_FATAL,
			"Unable to encrypt data-block "
			" src: %p srclen: %u buf: %p buflen: %u."
	...
	uint			buf_size)
{
	...
	int rc = encryption_scheme_decrypt(input_buf,
					   buf_size,
					   crypted_buf, &dstlen,
					   crypt_data,
					   crypt_data->key_version_alter,
	...
	ulint			space)	/*!< in: space id */
{
	...
	if (crypt_data) {
		if (!row_decrypt_buf(crypt_data, ofs, space,
				     buf, crypt_buf,
				     srv_sort_buf_size)) {
			return (FALSE);
		}
	...
	ulint		space)			/*!< in: space id */
{
	...
	if (crypt_data) {
		if (!row_encrypt_buf(crypt_data, ofs, space,
				(const byte *)buf,
				(byte *)crypt_buf, buf_len)) {
			return (FALSE);
		}
